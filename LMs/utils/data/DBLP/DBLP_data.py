...
def _tokenize_dblp_datasets(d):
    #! 创建目录
    if not osp.exists(osp.join(d.data_root, f'{d.DBLP_name}.txt')):
        mkdir_p(d.data_root)
        raise{'Please input'}
    #! Tokenize the data
    else:
        text = pd.read_csv(osp.join(d.data_root, f'{d.DBLP_name}.txt'), header=None, sep='\t')
        text = text[0]
        # Look at
    #! For debug
