...
def main():
    ...
    graph = dgl.load_graphs("/home/data/yh/Data/Amazon-Books-Children.pt")[0][0]
    labels = torch.unsqueeze(graph.ndata['label'], 1)
    torch.manual_seed(0)
    train_dataset, test_idx = random_split(
        dataset=range(116811),
        lengths=[93449, 23362],
    ...
    train_idx = torch.from_numpy(np.array(train_idx)).to(device)
    val_idx = torch.from_numpy(np.array(val_idx)).to(device)
    test_idx = torch.from_numpy(np.array(test_idx)).to(device)
