...
@torch.no_grad()
def evaluate(
    args, model, graph, feat, labels, train_idx, val_idx, test_idx, evaluator
):
    model.eval()
...
def main():
    ...
    # load feature
    if args.use_PLM:
        feat = torch.from_numpy(np.load(args.use_PLM).astype(np.float32)).to(device)
    ...
    for i in range(args.n_runs):
        ...
        val_acc, test_acc = run(
            args, graph, feat, labels, train_idx, val_idx, test_idx, evaluator, i + 1
        )
